...
The app is focused on DOCX to PDF and PDF printing. Discuss new formats and dependencies before implementing them. Avoid adding tests that only mirror view markup; test user-visible conversion and file-safety behavior.
...
For printing changes, run `./scripts/test-print.sh`. Test ordering, sheet boundaries, N-up, rasterization, capability rejection, and copy arguments without submitting jobs. Physical test printing is opt-in and must name the printer and paper/copy cost. Never send user documents as a test without explicit instruction.
